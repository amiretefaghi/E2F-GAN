...
def ProjectionHead_model(input_shape):
  input_1 = Input(shape= input_shape)
  input_g = GlobalAveragePooling2D()(input_1)

  d_1 = Dense(32)(input_g)
  ...
  model = keras.Model(inputs=input_1, outputs=d_3)
  return model
...
class GAN(tf.keras.Model):
    def __init__(self, image_shape = (256,256)):
        ...
        self.projectionhead = ProjectionHead_model(self.fine_encoder.output.shape[1:])
...
def build_networks (image_shape= (256,256) ,continue_training = False):
  gan = GAN(image_shape= image_shape)
  discriminator = Discriminator(input_shape=fine_image_shape)

  if continue_training == True:
    gan.fine_encoder.load_weights('./gan/fine_encoder_latest_weights.h5')
    gan.coarse_encoder.load_weights('./gan/coarse_encoder_latest_weights.h5')
    gan.decoder.load_weights('./gan/decoder_latest_weights.h5')
    gan.projectionhead.load_weights('./gan/projectionhead_latest_weights.h5')
    discriminator.load_weights('./gan/discriminator_latest_weights.h5')
...
def Contrastive_loss(hidden1,hidden2):
  labels = tf.one_hot(tf.range(hidden1.shape[0]), hidden1.shape[0] * 2)
  masks = tf.one_hot(tf.range(hidden1.shape[0]), hidden1.shape[0])

  temperature = 0.5
  logits_aa = tf.matmul(hidden1, hidden1, transpose_b=True) / temperature
  logits_aa = logits_aa - masks * 100
  logits_bb = tf.matmul(hidden2, hidden2, transpose_b=True) / temperature
  logits_bb = logits_bb - masks * 100
...
def train_g (gan: tf.keras.Model,
           ...
           epoch: int, pre_epoch: int) -> tf.Tensor:
  g_loss = 0
  d_loss = 0
  with tf.GradientTape() as tape1,tf.GradientTape() as tape2:
    ...
    f3 = gan.fine_encoder(image)
    hidden1 = gan.projectionhead(f1)
    hidden2 = gan.projectionhead(f3)

    output = gan.decoder([f1,f2])

    contrastive_loss = Contrastive_loss(hidden1,hidden2)
...
if __name__ == '__main__':
  ...
  for epoch in range(epochs):
    ...
    for step, (image,mask,scatter) in tqdm(enumerate(train_dataset)):
      if epoch <= pre_epoch:
        g_loss , d_loss = train_g(gan=gan,discriminator=discriminator,
                                ...
                                epoch=epoch,pre_epoch=pre_epoch)
      else:
    ...
    if epoch >= 5 and epoch%5 == 0:
      gan.fine_encoder.save_weights(f'./gan/fine_encoder_{epoch}_weights.h5')
      gan.coarse_encoder.save_weights(f'./gan/coarse_encoder_{epoch}_weights.h5')
      gan.decoder.save_weights(f'./gan/decoder_{epoch}_weights.h5')
      gan.projectionhead.save_weights(f'./gan/projectionhead_{epoch}_weights.h5')
      discriminator.save_weights(f'./gan/discriminator_{epoch}_weights.h5')

    gan.fine_encoder.save_weights(f'./gan/fine_encoder_latest_weights.h5')
    gan.coarse_encoder.save_weights(f'./gan/coarse_encoder_latest_weights.h5')
    gan.decoder.save_weights(f'./gan/decoder_latest_weights.h5')
    gan.projectionhead.save_weights(f'./gan/projectionhead_latest_weights.h5')
    discriminator.save_weights(f'./gan/discriminator_latest_weights.h5')
